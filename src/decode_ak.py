...
import math
import os
import pandas as pd # so that we could look at the binary in a csv file
...
#        print(val, pulse)
    ...
    if (counter < 5):
        ...
        if (counter == 4):
            ...
            print (temp_buf)
            temp_pandas = pd.DataFrame(temp_buf).T
            # now write to csv file
            temp_pandas.to_csv('decode.csv',mode='a',index = False ,header=False)
